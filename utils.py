...
def get_target_data(dataset, mode=None):
    image_list = []
    data = []
    help_texts = []  # 新增：用于存储辅助文本的列表

    # 统一将 dataset 转换为小写进行匹配，防止大小写引起的逻辑跳过
    dataset_lower = dataset.lower()

    if dataset_lower == "advbench":
        csv_file = os.path.join("dataset", "advbench", "harmful_behaviors.csv")
        df = pd.read_csv(csv_file)
        data = df["goal"].to_list()
        # 如果 CSV 中有 help_text 列则读取，否则填充空格
        help_texts = df["help_text"].to_list() if "help_text" in df.columns else [" "] * len(data)

    elif "safebench" in dataset_lower:
        # 修正路径匹配，确保能找到 SafeBench_Tiny.csv
        csv_file = os.path.join("dataset", "safebench", "question", dataset + ".csv")
        if not os.path.exists(csv_file):
            # 备选逻辑：如果找不到带后缀的，尝试直接找原名
            csv_file = os.path.join("dataset", "safebench", "question", "safebench.csv")

        df = pd.read_csv(csv_file)
        # 读取 question 列作为目标指令
        data = df["question"].to_list()
        # 读取 help_text 列作为辅助文本
        if "help_text" in df.columns:
            help_texts = df["help_text"].to_list()
        else:
            help_texts = [" "] * len(data)

    # 这里的返回逻辑必须与调用处 (llava_attack_help.py) 严格对应
    if mode == "mm":
        return data, image_list
    else:
        # 必须返回两个值，即使 help_texts 是空的
        return data, help_texts
